...
local unpack, table_concat, byte, char, string_rep, sub, string_format, floor, ceil, min, max = table.unpack or unpack,
				table.concat, string.byte, string.char, string.rep, string.sub, string.format, math.floor, math.ceil, math.min,
				math.max
...
local function sha256_feed_64(H, K, str, W, offs, size)
	...
	for pos = offs, size + offs - 1, 64 do
		...
		H[1], H[2], H[3], H[4] = (a + H[1]) % 4294967296, (b + H[2]) % 4294967296, (c + H[3]) % 4294967296,
						(d + H[4]) % 4294967296
		H[5], H[6], H[7], H[8] = (e + H[5]) % 4294967296, (f + H[6]) % 4294967296, (g + H[7]) % 4294967296,
						(h + H[8]) % 4294967296
	end
end
	...
	for pos = offs, size + offs - 1, 128 do
		...
		for jj = 17 * 2, 80 * 2, 2 do
			...
			W[jj - 1] = XOR(SHR(a_hi, 1) + SHL(a_lo, 31), SHR(a_hi, 8) + SHL(a_lo, 24), SHR(a_hi, 7)) +
							            XOR(SHR(b_hi, 19) + SHL(b_lo, 13), SHL(b_hi, 3) + SHR(b_lo, 29), SHR(b_hi, 6)) + W[jj - 15] +
							            W[jj - 33] + (tmp1 - tmp2) / 4294967296
			W[jj] = tmp2
		end
		local a_lo, b_lo, c_lo, d_lo, e_lo, f_lo, g_lo, h_lo, z_lo = H_lo[1], H_lo[2], H_lo[3], H_lo[4], H_lo[5], H_lo[6],
						H_lo[7], H_lo[8]
		local a_hi, b_hi, c_hi, d_hi, e_hi, f_hi, g_hi, h_hi, z_hi = H_hi[1], H_hi[2], H_hi[3], H_hi[4], H_hi[5], H_hi[6],
						H_hi[7], H_hi[8]
		for j = 1, 80 do
			local jj = 2 * j
			tmp1 = XOR(SHR(e_lo, 14) + SHL(e_hi, 18), SHR(e_lo, 18) + SHL(e_hi, 14), SHL(e_lo, 23) + SHR(e_hi, 9)) +
							       AND(e_lo, f_lo) + AND(-1 - e_lo, g_lo) + h_lo + K_lo[j] + W[jj]
			...
			b_lo = a_lo
			b_hi = a_hi
			tmp1 = z_lo + AND(d_lo, c_lo) + AND(b_lo, XOR(d_lo, c_lo)) +
...
for width = 224, 256, 32 do
	...
	sha512_feed_128(H_lo, H_hi, sha2_K_lo, sha2_K_hi,
					"SHA-512/" .. tonumber(width) .. "\128" .. string_rep("\0", 115) .. "\88", common_W, 0, 128)
...
local function sha512extBinary(width, text)
	...
	local function partial(text_part)
		if text_part then
			...
			if tail then
				...
				sha512_feed_128(H_lo, H_hi, sha2_K_lo, sha2_K_hi, final_blocks, common_W, 0, #final_blocks)
				local max_reg = ceil(width / 64)
				-- Combine H_hi and H_lo into a single binary string
				for j = 1, max_reg do
					H_lo[j] = string.pack(">I4I4", H_hi[j], H_lo[j]) -- Combine H_hi and H_lo as binary
				end
				H_hi = nil
				H_lo = table_concat(H_lo, "", 1, max_reg):sub(1, width / 8) -- Use width / 8 for binary length
			end
			return H_lo
		end
	end
	...
	if text then
		...
		-- Return function for partial chunk loading
		-- User should feed every chunks of input data as single argument to this function and receive SHA512 digest by invoking this function without an argument
		return partial
	end
...
local sha2for51 = {
	...
	sha512bin = function(text)
		return sha512extBinary(512, text)
	end, -- SHA-512 Binary
